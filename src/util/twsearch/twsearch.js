...
var node_fs_promises_unmangled = () =>
    node_fs_promises_mangled.replace(/-/g, "");
var wasm;
...
async function __wbg_init(input) {
    ...
    if (typeof input === "undefined") {
        throw new Error(
            "Default `wasm-pack` WASM loading code path triggered! This is currently not supported for `twsearch` due to incompatibility with some bundlers."
        );
    }
...
async function initWrapper() {
    await (cachedInitWrapper ??= (async () => {
        const wasmUint8Array = (await import("./twsearch_wasm_bg-BXMNNHLU.js"))
            .default;

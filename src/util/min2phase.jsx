...
const min2phase = (function () {
    ...
    CubieCube.prototype.invFrom = function (cc) {
        ...
        for (var corn = 0; corn < 8; corn++) {
            this.ca[cc.ca[corn] & 0x7] =
                corn | ((0x20 >> (cc.ca[corn] >> 3)) & 0x18);
        }
        return this;
    };
            ...
            Math.max(
                ...
                USE_TWIST_FLIP_PRUN
                    ? getPruningMax(
                          ...
                          (this.twist << 11) |
                              FlipS2RF[
                                  (this.flip << 3) | (this.fsym ^ this.tsym)
                              ]
                      )
                    : 0
            )
    ...
    Search.prototype.phase1PreMoves = function (maxl, lm, cc) {
        ...
        if (
            maxl == 1 ||
            this.preMoveLen + 1 + MIN_P1LENGTH_PRE >= this.length1
        ) {
            //last pre move
    ...
    Search.prototype.initPhase2Pre = function () {
        this.isRec = false;
        if (
            this.probe >= (this.moveSol == null ? this.probeMax : this.probeMin)
        ) {
            return 0;
        }
        ++this.probe;
    ...
    Search.prototype.initPhase2 = function (phase2Cubie) {
        ...
        var prun = Math.max(
            ...
            getPruningMax(
                ...
                EPermCCombPPrun,
                p2edge * N_COMB +
                    CCombPConj[Perm2CombP[p2corn] & 0xff][
                        SymMultInv[p2esym][p2csym]
                    ]
            ),
            getPruningMax(
                EPermCCombPPrunMax,
                EPermCCombPPrun,
                (p2edgei >> 4) * N_COMB +
                    CCombPConj[Perm2CombP[p2corni >> 4] & 0xff][
                        SymMultInv[p2edgei & 0xf][p2corni & 0xf]
                    ]
            )
        );
    ...
    Search.prototype.appendSolMove = function (curMove) {
        ...
        if (axisCur == axisLast) {
            var pow =
                ((curMove % 3) +
                    (this.moveSol[this.moveSol.length - 1] % 3) +
                    1) %
                4;
            if (pow == 3) {
                this.moveSol.pop();
        ...
        if (
            ...
            var pow =
                ((curMove % 3) +
                    (this.moveSol[this.moveSol.length - 2] % 3) +
                    1) %
                4;
            if (pow == 3) {
        ...
        for (var m = 0; m < 10; m++) {
            ...
            prun = Math.max(
                getPruningMax(
                    ...
                    EPermCCombPPrun,
                    edgex * N_COMB +
                        CCombPConj[Perm2CombP[cornx] & 0xff][
                            SymMultInv[esymx][csymx]
                        ]
                ),
                getPruningMax(
                    MCPermPrunMax,
    ...
    function initRawSymPrun(
        ...
        var SYM_E2C_MAGIC =
            ((PrunFlag >> 4) & 1) == 1 ? 0x00dddd00 : 0x00000000;
